...
通常の run では到達しない分岐 (sudo キープアライブの 60 秒リフレッシュ、watchdog の
...
拒否する — 実際それで一度 15/15 と自信満々に報告し、真の数字は 10/15 だった。
`--show` は各ミューテーションが実際に何を変えるかを出す。これで「何もテストして
いなかったミューテーション」が 2 つ見つかった。
